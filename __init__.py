...
class BackgroundQueueProcessingModalMixin:
    ...
    # NOTE: More optional callbacks.. excepted signature: (self, context) & return None
    callback_before_queue = None #NOTE: This callback could be used to build self.queues[self.queue_identifier][..][..] if needed.
    callback_after_queue = None  #NOTE: This callback is to be used to handle the queue after it has been successfully executed.
    callback_fatal_error = None  #NOTE: This callback is to be used to handle fatal errors, errors that would cancel out the whole queue.
    ...
    def execute(self, context):
        ...
        try:            
            ...
            if (len(self.qactive) != valid_worker_found):
                print(f"ERROR: {self._debugname}.execute(): We couldn't find all the worker functions for your queue.")
                self.cleanup(context)
                return {'FINISHED'}
